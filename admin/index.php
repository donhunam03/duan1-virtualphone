    ...
    switch ($act) {
        ...
        case 'update_bill':
            if (isset($_POST['btn_update']) && ($_POST['btn_update'])) {
                ...
                update_bill($id_bill,$status_pay);
                echo '<script>alert("Cập nhật đơn hàng thành công!")</script>';
                header('location:index.php?act=list_bill');
            }
            break;
            case  "deleted_bill_admin";
                deleted_bill($_GET['id']);
                echo '<script>alert("Xóa thành công!");</script>';
                header('Location:?act=list_bill');
            break;
            case "update_giao_admin";
            update_giao($_GET['id']);
            echo '<script>alert("Xác nhận thành công!");</script>';
            header('Location:?act=list_bill');
            break;
           case "update_nhan_admin";
            update_nhan($_GET['id']);
            echo '<script>alert("Xác nhận thành công!");</script>';
            header('Location:?act=list_bill');
            break;
